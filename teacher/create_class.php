...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $insert_subject->bind_param("ss", $subject_code, $subject_name);
        if ($insert_subject->execute()) {
            $subject_id = $insert_subject->insert_id;
        } else {
            $_SESSION['error_message'] = "Error creating subject: " . $insert_subject->error;
    ...
    $stmt->bind_param("sss", $section_name, $academic_year, $semester);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) {
        ...
        $insert_section->bind_param("sss", $section_name, $academic_year, $semester);
        if ($insert_section->execute()) {
            $section_id = $insert_section->insert_id;
        } else {
            $_SESSION['error_message'] = "Error creating section: " . $insert_section->error;
            header("Location: create_class.php");
            exit();
        }
    ...
    if ($result->num_rows > 0) {
        $_SESSION['error_message'] = "A class with this subject and section already exists for you.";
        header("Location: create_class.php");
        exit();
    }
    $stmt->close();
    ...
    $insert_class->bind_param("iiis", $teacher_id, $subject_id, $section_id, $grading_type);
    ...
    if ($insert_class->execute()) {
        ...
        $_SESSION['error_message'] = "Error creating class: " . $insert_class->error;
        header("Location: create_class.php");
        exit();
    }
    $insert_class->close();
...
<head>
    ...
    <title>Create Class - Universidad De Manila</title>
                        ...
                        <div class="col-md-6">
                         ...
                         <div class="col-md-6">
                            ...
                            <select class="form-select" id="grading_system_type" name="grading_system_type" required>
                                ...
                                <option value="a_na">A/NA Based</option>
                                </select>
                            <div class="invalid-feedback">
