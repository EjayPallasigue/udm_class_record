...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($subject_code) || empty($subject_name) || empty($section_name) || empty($academic_year) || empty($semester) || empty($grading_system_type)) {
        ...
        $subject_check_stmt->bind_param("s", $subject_code);
        ...
        if ($subject_result->num_rows > 0) {
            ...
            $update_subject_stmt->bind_param("si", $subject_name, $subject_id);
            ...
            $subject_insert_stmt->bind_param("ss", $subject_code, $subject_name);
            $subject_insert_stmt->execute();
            $subject_id = $conn->insert_id;
            $subject_insert_stmt->close();
        }
        ...
        $section_check_stmt->bind_param("sss", $section_name, $academic_year, $semester);
        $section_check_stmt->execute();
        $section_result = $section_check_stmt->get_result();
        ...
        if ($section_result->num_rows > 0) {
            ...
            $section_insert_stmt->bind_param("sss", $section_name, $academic_year, $semester);
            $section_insert_stmt->execute();
            $section_id = $conn->insert_id;
            $section_insert_stmt->close();
        }
        $section_check_stmt->close();
        ...
        $check_stmt->bind_param("iiii", $subject_id, $section_id, $teacher_id, $class_id);
        $check_stmt->execute();
        ...
        if ($check_row['count'] > 0) {
            ...
            $update_stmt->bind_param("iisii", $subject_id, $section_id, $grading_system_type, $class_id, $teacher_id);
            ...
            if ($update_stmt->execute()) {
                ...
                if ($grading_system_type === 'final_only_numerical') {
                    ...
                    $delete_components_stmt->bind_param("i", $class_id);
                    $delete_components_stmt->execute();
                    $delete_components_stmt->close();
                    ...
                    $is_locked = 0;
                    $prelim_weight = 0.00;
                    $prelim_stmt->bind_param("isssdiid", $class_id, $prelim_name, $prelim_period, $prelim_type, $prelim_max_score, $is_attendance, $is_locked, $prelim_weight);
                    $prelim_stmt->execute();
                    $prelim_stmt->close();
                    ...
                    $midterm_stmt->bind_param("isssdiid", $class_id, $midterm_name, $midterm_period, $midterm_type, $midterm_max_score, $is_attendance, $is_locked, $prelim_weight);
                    ...
                    $is_manual = 0;
                    $final_weight = 100.00;
                    $final_stmt->bind_param("isssdiid", $class_id, $final_name, $final_period, $final_type, $final_max_score, $is_manual, $is_locked, $final_weight);
                    $final_stmt->execute();
                    $final_stmt->close();
                }
...
<head>
    ...
    <title>Edit Class - Universidad De Manila</title>
            ...
            <div class="card-body">
                <form method="POST" action="" class="needs-validation" novalidate>
                    ...
                    <div class="mb-4">
                        ...
                        <!-- A/NA-Based Grading Information -->
