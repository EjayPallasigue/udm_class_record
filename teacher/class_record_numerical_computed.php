...
$components_sql = "SELECT component_id, component_name, max_score, period, is_attendance_based FROM grade_components WHERE class_id = ? ORDER BY FIELD(period, 'Preliminary', 'Mid-Term', 'Pre-Final'), component_name";
$components_stmt = $conn->prepare($components_sql);
$components_stmt->bind_param("i", $class_id);
$components_stmt->execute();
$components_query_result = $components_stmt->get_result();
$all_components_for_class = [];
while ($comp = $components_query_result->fetch_assoc()) {
    $all_components_for_class[] = $comp;
}
...
if ($total_students > 0 && !empty($all_components_for_class)) {
    ...
    if (!empty($enrollment_ids)) {
        ...
        $grades_stmt->bind_param($types, ...$enrollment_ids);
        $grades_stmt->execute();
        $grades_query_result = $grades_stmt->get_result();
        while ($grade = $grades_query_result->fetch_assoc()) {
...
<head>
    ...
    <title>Computed Class Record - <?= htmlspecialchars($class['subject_name'] ?? 'Class') ?> - Universidad De Manila</title>
...
<body>
    <div class="main-wrapper">
        ...
        <main class="content-area">
            <header class="page-header">
                ...
                <div class="page-actions flex-wrap"> <a href="input_grades_numerical.php?class_id=<?= $class_id ?>" class="btn btn-primary">
                        <i class="bi bi-pencil-square me-1"></i> Input Grades
                    </a>
                    ...
                    <form method="get" class="d-flex ms-lg-auto mt-2 mt-lg-0"> <input type="hidden" name="class_id" value="<?= $class_id ?>">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="bi bi-file-earmark-spreadsheet me-2"></i>Students (<?= $total_students ?>)</h5>
                     <span class="badge bg-primary rounded-pill">Numerical Grading System</span>
                </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a href="../public/logout.php" class="btn btn-danger">Logout Directly</a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Chatbot functionality (existing)
        const chatbotToggle = document.getElementById('chatbotToggle');
        const chatbotPopover = new bootstrap.Popover(chatbotToggle, {
            html: true,
            sanitize: false, 
            content: document.getElementById('chatbotContent').innerHTML,
            placement: 'left',
            trigger: 'click',
            container: 'body'
        });

        const CHAT_STORAGE_KEY = 'chatbotConversation_computed'; // Use a different key if needed
        let chatbotMessages = null; 

        chatbotToggle.addEventListener('shown.bs.popover', function () {
            chatbotMessages = document.querySelector('.popover.show #chatbotMessages'); // More specific selector
            if (chatbotMessages) {
                loadConversation();
                document.querySelector('.popover.show #chatbotSendBtn').addEventListener('click', sendMessage);
                document.querySelector('.popover.show #chatbotInput').addEventListener('keypress', function (e) {
                    if (e.key === 'Enter') {
                        e.preventDefault(); 
                        sendMessage();
                    }
                });
                document.querySelector('.popover.show #clearChatBtn').addEventListener('click', clearChat);
                // Close button inside popover
                const closePopoverButton = document.querySelector('.popover.show .btn-close-popover');
        ...
        if(mainLogoutButton){
            mainLogoutButton.addEventListener('click', function() {
                ...
                // The actual logout happens when "Logout Directly" in modal is clicked.
            });
        }
        ...
        function sendMessage() {
            const inputField = document.querySelector('.popover.show #chatbotInput');
            if (!inputField || !chatbotMessages) return;
            ...
            if (messageText) {
                ...
                const typingIndicator = addMessage('Isla', '<span class="typing-indicator-dot">.</span><span class="typing-indicator-dot">.</span><span class="typing-indicator-dot">.</span>', 'isla-message typing-indicator');
                if(chatbotMessages && typingIndicator) chatbotMessages.scrollTop = chatbotMessages.scrollHeight;

                simulateGeminiResponse(messageText).then(response => {
                    if(chatbotMessages && typingIndicator && chatbotMessages.contains(typingIndicator)) chatbotMessages.removeChild(typingIndicator);
                    addMessage('Isla', response, 'isla-message');
                    saveConversation();
                }).catch(error => {
                    console.error('Error fetching chatbot response:', error);
                    if(chatbotMessages && typingIndicator && chatbotMessages.contains(typingIndicator)) chatbotMessages.removeChild(typingIndicator);
                    addMessage('Isla', 'Sorry, I am having trouble connecting. Please try again later.', 'isla-message');
                    saveConversation();
                });
            }
        }
        ...
        function addMessage(sender, text, type) {
            ...
            messageContainer.innerHTML = `<p><strong>${sender}:</strong> ${text}</p>`;
        ...
        async function simulateGeminiResponse(userMessage) {
            if (userMessage.toLowerCase().includes('hello')) {
                return "Hello! How can I assist you with your class records today?";
            } else if (userMessage.toLowerCase().includes('grades')) {
                return "You can input and view student grades for various components and periods in the 'Input Grades' and 'Computed Class Record' sections.";
            } else if (userMessage.toLowerCase().includes('computed grades')) {
                return "The computed grades show the preliminary, mid-term, pre-final, and final computed grades based on the UDM grading system formulas (60% Class Standing, 40% Exam for period grades; 30% Prelim, 30% Mid-Term, 40% Pre-Final for final grade).";
            } else if (userMessage.toLowerCase().includes('manage components')) {
                return "The 'Manage Components' section allows you to define and configure grade components like quizzes, assignments, and exams, including their maximum scores and the period they belong to.";
            }
        ...
        function printComputedGradesTable() {
            ...
            if (!tableToPrint) {
                alert('Computed grades table not found!'); // Simple alert for error
                return;
            }
           ...
           const teacherName = "<?= htmlspecialchars(isset($_SESSION['teacher_name']) ? $_SESSION['teacher_name'] : 'N/A', ENT_QUOTES, 'UTF-8') ?>"; // Assuming teacher name is in session
            ...
            // Basic print styles - these will be complemented by @media print CSS
            ...
            printWindow.document.write('.print-info-table td { padding: 4px; border: none; text-align:left; }'); // No borders for info table cells
            printWindow.document.write('table { width: 100%; border-collapse: collapse; margin-top: 10px; }');
            printWindow.document.write('th, td { border: 1px solid #000; padding: 3px 5px; text-align: center; vertical-align: middle; font-size: 8pt; }'); // General table cell style
            printWindow.document.write('thead th { background-color: #e9e9e9; font-weight: bold; font-size: 8pt; }'); // Header specific
            printWindow.document.write('tbody td { font-size: 8pt; }');
            printWindow.document.write('.student-name { font-weight: normal; font-size: 9pt; text-align: left !important; }'); // Student name style
            printWindow.document.write('.student-id { font-size: 7.5pt; color: #333; text-align: left !important; display: block; }'); // Student ID style
            printWindow.document.write('.text-start { text-align: left !important; }'); // Ensure text-start is applied
            printWindow.document.write('.bg-success-subtle { background-color: #e6ffed !important; color: #000 !important; }'); // Ensure final grade highlight
            ...
            // Ensure student names and IDs are properly styled in the clone if specific selectors were missed by general CSS
            ...
            // Make sure header cells in the cloned table also have correct text alignment if needed
            clonedTable.querySelectorAll('thead th').forEach(th => {
                 th.style.textAlign = 'center'; // Default for headers
                 if (th.classList.contains('text-start')) { // Override for student info header
            ...
            // Use a timeout to ensure content is loaded before printing in some browsers
            setTimeout(function() {
                printWindow.print();
    ...
    <div class="chatbot-container">
        <button type="button" class="btn btn-primary btn-chatbot" id="chatbotToggle" data-bs-toggle="popover" title="Chat with Isla">
            <i class="bi bi-chat-dots-fill"></i>
        </button>
    </div>

    <div id="chatbotContent" style="display: none;">
        <div class="chatbot-messages" id="chatbotMessages">
            </div>
        <div class="input-group mb-3">
            <input type="text" id="chatbotInput" class="form-control" placeholder="Type your message..." aria-label="Type your message">
        ...
        <div class="d-flex justify-content-between">
            ...
            <button class="btn btn-sm btn-outline-danger btn-close-popover" type="button">Close</button> </div>
    </div>
</body>
</html>