...
function log_history($conn, $class_id, $enrollment_id, $teacher_id, $grade_type, $old_value, $new_value) {
    ...
    $stmt->bind_param("iiisss", $class_id, $enrollment_id, $teacher_id, $grade_type, $old_value, $new_value);
    $stmt->execute();
    $stmt->close();
}
...
function update_attendance($conn, $enrollment_id, $component_id, $new_status, $class_id, $teacher_id, $period) {
    ...
    // Fetch existing attendance statuses for this enrollment_id from student_grades table
    // Assuming student_grades has a unique key on (enrollment_id) for attendance data,
    // and prelim/midterm statuses are in the same row.
    // MODIFICATION START: Fetching existing attendance values more robustly
    $stmt_old_data = $conn->prepare("SELECT attendance_status_prelim, attendance_status_midterm FROM student_grades WHERE enrollment_id = ?");
    $stmt_old_data->bind_param("i", $enrollment_id);
    $stmt_old_data->execute();
    $result_old_data = $stmt_old_data->get_result();
    if ($row = $result_old_data->fetch_assoc()) {
    ...
    if ($status_changed) {
        // Use a comprehensive INSERT...ON DUPLICATE KEY UPDATE statement
        // to update both attendance columns and the component_id (which will represent the last updated attendance type).
        // This ensures all attendance data for the enrollment_id is maintained in one row.
        // MODIFICATION START: Ensure component_id is set for attendance components
        $stmt_op = $conn->prepare("
            ...
            ON DUPLICATE KEY UPDATE
                ...
                attendance_status_prelim = VALUES(attendance_status_prelim),
                attendance_status_midterm = VALUES(attendance_status_midterm)
        ");
        $stmt_op->bind_param("iiss", $enrollment_id, $component_id, $current_prelim_status, $current_midterm_status);
        $stmt_op->execute();
        $stmt_op->close();
...
function update_final_grade($conn, $enrollment_id, $new_final_grade, $class_id, $teacher_id) {
    ...
    $stmt_old->bind_param("ii", $enrollment_id, $class_id);
    $stmt_old->execute();
    if ($row = $stmt_old->get_result()->fetch_assoc()) $old_final_grade = $row['overall_final_grade'];
    $stmt_old->close();

    if ((string)$old_final_grade !== (string)$new_final_grade || ($old_final_grade === null && $new_final_grade !== 0.0)) {
        $stmt_op = $conn->prepare("INSERT INTO final_grades (enrollment_id, class_id, overall_final_grade, remarks) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE overall_final_grade = VALUES(overall_final_grade), remarks = VALUES(remarks)");
        $stmt_op->bind_param("iids", $enrollment_id, $class_id, $new_final_grade, $remarks);
        $stmt_op->execute();
        $stmt_op->close();
...
$required_components_config = [
    'Preliminary_Attendance' => ['period' => 'Preliminary', 'is_attendance_based' => 1, 'name' => 'Attendance - Preliminary', 'type' => 'Attendance_Component', 'max_score' => 0, 'default_locked' => 0],
    'Mid_Term_Attendance'    => ['period' => 'Mid-Term', 'is_attendance_based' => 1, 'name' => 'Attendance - Mid-Term', 'type' => 'Attendance_Component', 'max_score' => 0, 'default_locked' => 1],
    'Pre_Final_Grade'        => ['period' => 'Pre-Final', 'is_attendance_based' => 0, 'name' => 'Pre-Final Grade', 'type' => 'Numerical_Component', 'max_score' => 100, 'default_locked' => 1]
];
...
foreach ($required_components_config as $key => $config) {
    ...
    $stmt->bind_param("isi", $class_id, $config['period'], $config['is_attendance_based']);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($row = $res->fetch_assoc()) {
        ...
        $stmt_insert->bind_param("isssiii", $class_id, $config['name'], $config['period'], $config['type'], $config['max_score'], $config['is_attendance_based'], $config['default_locked']);
        $stmt_insert->execute();
        $grade_components_status[$key] = ['component_id' => $stmt_insert->insert_id, 'is_locked' => (bool)$config['default_locked']];
        $stmt_insert->close();
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_lock_component'])) {
    ...
    $stmt_verify = $conn->prepare("SELECT gc.component_id FROM grade_components gc JOIN classes c ON gc.class_id = c.class_id WHERE gc.component_id = ? AND c.class_id = ? AND c.teacher_id = ?");
    $stmt_verify->bind_param("iii", $component_id, $class_id, $teacher_id);
    $stmt_verify->execute();
    if ($stmt_verify->get_result()->num_rows === 0) {
        echo json_encode(['success' => false, 'message' => 'Unauthorized or component not found.']);
        exit();
    }
    $stmt_verify->close();

    $stmt_update = $conn->prepare("UPDATE grade_components SET is_locked = ? WHERE component_id = ?");
    $stmt_update->bind_param("ii", $is_locked, $component_id);
    echo json_encode($stmt_update->execute()
        ? ['success' => true, 'is_locked' => (bool)$is_locked]
        : ['success' => false, 'message' => 'Failed to update lock status: ' . $conn->error]);
    $stmt_update->close();
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['toggle_lock_component'])) {
    ...
    try {
        ...
        $stmt_locks->bind_param("i", $class_id);
        $stmt_locks->execute();
        ...
        foreach ($_POST['grades'] as $enrollment_id => $data) {
            foreach (['Preliminary', 'Mid-Term'] as $period) {
                ...
                // MODIFICATION START: Ensure the component_id exists for attendance updates
                $comp_id_for_attendance = $component_ids[$key] ?? null;
                if ($comp_id_for_attendance && !($current_lock_status[$key] ?? true) && isset($data[$period])) {
...
$existing_attendance = [];
// MODIFICATION START: Corrected data fetching for existing attendance.
// This assumes attendance_status_prelim and attendance_status_midterm are columns in the same student_grades row
// for a given enrollment_id.
$stmt = $conn->prepare("
    SELECT
        sg.enrollment_id,
    ...
    WHERE
        e.class_id = ?
        AND (sg.attendance_status_prelim IS NOT NULL OR sg.attendance_status_midterm IS NOT NULL)
"); // Removed component_id filter from here to get all attendance for enrollment_id
$stmt->bind_param("i", $class_id);
$stmt->execute();
...
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) $raw_history_logs[] = $row;
$stmt->close();
    ...
    <main class="content-area">
        <header class="page-header">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <button type="button" class="btn btn-outline-info" data-bs-toggle="modal" data-bs-target="#historyModal"><i class="bi bi-clock-history"></i> History</button>
                <a href="view_class_record.php?class_id=<?= $class_id ?>" class="btn btn-outline-primary"><i class="bi bi-table"></i> Class Record</a>
                <button type="button" class="btn btn-outline-secondary" id="printGradesButton"><i class="bi bi-printer"></i> Print Grades</button>
        ...
        <?php else: ?>
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary rounded-pill"><?= $students_result->num_rows ?> Student<?= $students_result->num_rows > 1 ? 's' : '' ?></span>
                </div>
                    ...
                    <form action="<?= $_SERVER['PHP_SELF'] ?>?class_id=<?= $class_id ?>" method="POST" id="gradesForm">
                        ...
                        <div class="sticky-action-bar text-end">
                            <div class="container-fluid px-4">
                                <button type="submit" class="btn btn-primary"><i class="bi bi-save me-2"></i> Save All Grades</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<div class="modal fade" id="logoutModal" tabindex="-1" aria-labelledby="logoutModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-warning-subtle"><h5 class="modal-title" id="logoutModalLabel">Confirm Logout</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
      <div class="modal-body"><p>Would you like to save the database before logging out?</p></div>
      <div class="modal-footer">
        <form action="../public/export_db.php" method="POST" class="d-inline"><button type="submit" class="btn btn-success">Save Database</button></form>
        <a href="../public/logout.php" class="btn btn-danger">Logout</a><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
      </div>
    </div>
  </div>
</div>
        ...
        document.querySelectorAll('.lock-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                fetch('<?= $_SERVER['PHP_SELF'] ?>?class_id=<?= $class_id ?>', {
                    ...
                    body: new URLSearchParams({ toggle_lock_component: '1', component_id: componentId, is_locked: newLockStatus })
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.querySelectorAll(`[data-period="${period}"]`).forEach(input => input.disabled = newLockStatus);
                        showAlert('success', `Component ${period.replace(/_/g, ' ')} ${newLockStatus ? 'LOCKED' : 'UNLOCKED'}.`);
                    } else {
                        showAlert('danger', `Error: ${data.message || 'Failed to update.'}`);
                    }
                })
                .catch(error => showAlert('danger', 'An unexpected error occurred.'));
            });
        });
    ...
    function loadConversation() {
        if (chatbotMessages) {
            ...
            if (savedConversation) {
                ...
                // Initial welcome message for Isla
