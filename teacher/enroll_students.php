...
function processStudentData($conn, $student_number, $last_name, $first_name, $class_id, &$enrolled_count, &$error_count, &$skipped_count) {
    ...
    $res_student = $stmt_check_student->get_result();
    ...
    if ($res_student->num_rows > 0) {
        ...
        $stmt_insert_student->bind_param("sss", $student_number, $last_name, $first_name);
        if ($stmt_insert_student->execute()) {
            $student_id = $stmt_insert_student->insert_id;
        } else {
            $error_count++;
    ...
    if (isset($stmt_insert_student)) $stmt_insert_student->close();
    ...
    $already_enrolled = $stmt_check_enrollment->get_result()->num_rows > 0;
    ...
    if (!$already_enrolled) {
        ...
        $stmt_enroll->bind_param("ii", $student_id, $class_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_bulk_text'])) { // <--- MODIFIED: Added specific submit button name
    $enrolled_count = 0;
    $error_count = 0;
...
");
$stmt_fetch_students->bind_param("i", $class_id);
$stmt_fetch_students->execute();
$students_result = $stmt_fetch_students->get_result();
...
<head>
    ...
    <title>Enroll Students - Universidad De Manila</title>
            ...
            <div class="col-lg-5 mb-4">
                ...
                <div class="card shadow-sm mt-4">
                    ...
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data"> <div class="mb-3">
                                <label for="excel_file" class="form-label">Upload Excel file (.xls, .xlsx, .ods):</label>
                                <input class="form-control" type="file" id="excel_file" name="excel_file" accept=".xls,.xlsx,.ods">
                                <div class="form-text">
                                    <i class="bi bi-info-circle-fill me-1"></i> File should have columns: Student Number (Col A), Last Name (Col B), First Name (Col C). Data should start from the first row or second if a header is present.
                                </div>
                            </div>
                            <div class="d-grid">
                ...
                <div class="card shadow-sm mt-4">
                    ...
                    <div class="card-body">
                        ...
                        <ol class="ps-3 mb-3">
                            <li>Prepare an Excel file (.xls, .xlsx).</li>
