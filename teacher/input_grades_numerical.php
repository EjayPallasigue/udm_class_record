...
$stmt = $conn->prepare("SELECT e.enrollment_id, s.student_number, s.first_name, s.last_name
                        ...
                        WHERE e.class_id = ? ORDER BY s.last_name, s.first_name");
$stmt->bind_param("i", $class_id);
$stmt->execute();
$students_result = $stmt->get_result(); // Keep as result object for looping in HTML & num_rows check
$stmt->close();

// Fetch grade components
$stmt = $conn->prepare("SELECT component_id, class_id, component_name, max_score, period, is_attendance_based FROM grade_components WHERE class_id = ? ORDER BY period, component_name");
$stmt->bind_param("i", $class_id);
$stmt->execute();
$component_list = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
$existing_grades = [];
if ($students_result->num_rows > 0 && !empty($component_list)) {
    $stmt = $conn->prepare("SELECT sg.enrollment_id, sg.component_id, sg.score
                            FROM student_grades sg
                            JOIN enrollments e ON sg.enrollment_id = e.enrollment_id
                            WHERE e.class_id = ?");
    $stmt->bind_param("i", $class_id);
    $stmt->execute();
    $grades_query_result = $stmt->get_result();
    ...
    <main class="content-area">
        <header class="page-header">
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                <button type="button" class="btn btn-outline-info" id="printGradesButtonNumerical"><i class="bi bi-printer"></i> Print Grades</button>
        ...
        <?php else: ?>
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body p-0">
                    <form action="save_grades_regular.php" method="POST" id="gradesForm">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover table-sticky grades-table mb-0" id="numericalGradesInputTable">
                                ...
                                <tbody>
                                    <?php
                                    mysqli_data_seek($students_result, 0); // Rewind student results for iteration
                                    ...
                                    <?php endwhile; // End students_result loop ?>
                                </tbody>
                            </table>
                        </div>
                        <div class="sticky-action-bar text-end">
                            <div class="container-fluid px-4"> <button type="submit" class="btn btn-primary"><i class="bi bi-save me-2"></i> Save All Grades</button>
                            </div>
                        </div>
                    </form>
                </div>
