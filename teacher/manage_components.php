...
$class_query->bind_param("ii", $class_id, $teacher_id);
$class_query->execute();
...
if (isset($_GET['delete_id']) && is_numeric($_GET['delete_id'])) {
    ...
    $check_query = $conn->prepare("SELECT gc.component_id FROM grade_components gc 
                                  ...
                                  WHERE gc.component_id = ? AND gc.class_id = ? AND c.teacher_id = ?");
    $check_query->bind_param("iii", $delete_id, $class_id, $teacher_id);
    $check_query->execute();
    $check_result = $check_query->get_result();
    
    if ($check_result->num_rows > 0) {
        $delete_stmt = $conn->prepare("DELETE FROM grade_components WHERE component_id = ?");
        ...
        if ($delete_stmt->execute()) {
            ...
            if ($conn->errno == 1451) { 
                $message = "Cannot delete this component because it has associated student grades. Please remove or reassign the grades first.";
            } else {
                $message = "Error deleting component: " . $conn->error;
            }
            $message_type = "danger";
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['edit_component_id']) && is_numeric($_POST['edit_component_id'])) {
        ...
        // Fix: Explicitly set to NULL if empty string
        ...
        $check_query = $conn->prepare("SELECT gc.component_id FROM grade_components gc
                                      JOIN classes c ON gc.class_id = c.class_id
                                      WHERE gc.component_id = ? AND gc.class_id = ? AND c.teacher_id = ?");
        $check_query->bind_param("iii", $edit_id, $class_id, $teacher_id);
        $check_query->execute();
        $check_result = $check_query->get_result();

        if ($check_result->num_rows > 0) {
            $name_check = $conn->prepare("SELECT component_id FROM grade_components
                                          WHERE class_id = ? AND component_name = ? AND component_id != ?");
            $name_check->bind_param("isi", $class_id, $edit_component_name, $edit_id);
            $name_check->execute();
            ...
            if ($name_result->num_rows > 0) {
                ...
                // New: Added weight to the update statement
                // Fix: 's' for string if period/type can be NULL, or 's' for string if always string
                // The correct type for NULL values in bind_param is 's' (string) when using non-strict mode,
                // or you need to use bind_param_array if you are using mysqli_stmt::execute_array or need to handle types dynamically.
                // For simplicity and common practice, 's' works for strings including null in many scenarios.
                $update_stmt = $conn->prepare("UPDATE grade_components
                                             SET component_name = ?, max_score = ?,
                                             period = ?, type = ?, is_attendance_based = ?, weight = ?
                                             WHERE component_id = ?");
                // New: Added 'd' for double/decimal for weight, and $edit_weight
                $update_stmt->bind_param("sissidi", $edit_component_name, $edit_max_score,
                                       $edit_period, $edit_type, $edit_is_attendance_based, $edit_weight, $edit_id);

                if ($update_stmt->execute()) {
                    // Redirect to avoid re-submission and to show clean URL
                    header("Location: manage_components.php?class_id=$class_id&success_edit=1");
                    exit();
                } else {
                    $message = "Error updating component: " . $conn->error;
                    $message_type = "danger";
                }
            }
        } else {
        ...
        $check = $conn->prepare("SELECT * FROM grade_components WHERE class_id = ? AND component_name = ?");
        $check->bind_param("is", $class_id, $component_name);
        $check->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            // New: Added weight to the insert statement
            // Fix: 's' for string if period/type can be NULL
            $stmt = $conn->prepare("INSERT INTO grade_components (class_id, component_name, max_score, period, type, is_attendance_based, weight) VALUES (?, ?, ?, ?, ?, ?, ?)");
            // New: Added 'd' for double/decimal for weight, and $weight
            $stmt->bind_param("isissid", $class_id, $component_name, $max_score, $period, $type, $is_attendance_based, $weight);

            if ($stmt->execute()) {
                header("Location: manage_components.php?class_id=$class_id&success=1");
                exit();
            } else {
                $message = "Error adding component: " . $conn->error;
                $message_type = "danger";
            }
        }
    }
...
// Get existing components (assuming 'weight' column exists)
$stmt = $conn->prepare("SELECT * FROM grade_components WHERE class_id = ? ORDER BY period, component_name"); // Added period to order
...
// Group components by Period for better display reflecting the grading structure
$grouped_components = [];
if (!empty($components)) {
    ...
    $grouped_components = [
        // 'All Components' => [] // Keep this or adjust based on new grouping
    ];
}
...
<head>
    ...
    <title>Manage Grade Components - Universidad De Manila</title>
...
<div class="main-wrapper">
    ...
    <main class="content-area">
        ...
        data-weight="<?= htmlspecialchars(number_format((float)($component['weight'] ?? 0), 2)) ?>"> 
                                                            ...
                                                            <a href="?class_id=<?= $class_id ?>&delete_id=<?= $component['component_id'] ?>" 
                                                               class="btn btn-outline-danger delete-btn"
                                                               onclick="return confirm('Are you sure you want to delete this component? This action cannot be undone.');">
