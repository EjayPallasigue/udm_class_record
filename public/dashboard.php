...
require_once '../config/db.php'; // Assuming this connects to $conn
...
// Function to check if table exists - Fixed version
function tableExists($conn, $tableName) {
    // Method 1: Use INFORMATION_SCHEMA (more reliable)
    $query = "SELECT COUNT(*) as count FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ?";
    $stmt = $conn->prepare($query);
    if (!$stmt) {
        // Fallback method if prepare fails
...
// Alternative simpler function if the above still causes issues
function tableExistsSimple($conn, $tableName) {
    $escapedTableName = $conn->real_escape_string($tableName);
    $query = "SHOW TABLES LIKE '$escapedTableName'";
    $result = $conn->query($query);
    return $result && $result->num_rows > 0;
...
function safeDeleteFromTable($conn, $tableName, $whereClause, $params, $paramTypes) {
    ...
    if (!$success) {
        error_log("Failed to delete from table '$tableName': " . $stmt->error);
    }
    $stmt->close();
    return $success;
}
...
if (isset($_POST['delete_class']) && isset($_POST['class_id'])) {
    ...
    try {
        ...
        if (!safeDeleteFromTable($conn, 'grades', 'class_id = ?', [$class_id], 'i')) {
            $allSuccessful = false;
        }

        // Check and delete from enrollments table
        if (!safeDeleteFromTable($conn, 'enrollments', 'class_id = ?', [$class_id], 'i')) {
            $allSuccessful = false;
        }

        // Check and delete from grade_components table
        if (!safeDeleteFromTable($conn, 'grade_components', 'class_id = ?', [$class_id], 'i')) {
            $allSuccessful = false;
        }

        // Finally, delete the class (this table should exist)
        if ($allSuccessful) {
            if (tableExists($conn, 'classes')) {
                ...
                if ($stmt) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $conn->rollback();
                        $_SESSION['error_message'] = "Error deleting class: " . $stmt->error;
                    }
                    $stmt->close();
                } else {
                    $conn->rollback();
...
if (isset($_POST['delete_note']) && isset($_POST['note_number'])) {
    ...
    // Fetch all notes first to get their IDs
    $sql_notes = "SELECT note_id FROM notes ORDER BY reg_date DESC";
    ...
    if ($result_notes) {
        ...
        echo json_encode(['status' => 'error', 'message' => 'Error fetching notes for deletion: ' . $conn->error]);
        exit();
    }
    ...
    $delete_note_sql = "DELETE FROM notes WHERE note_id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                ...
                echo json_encode(['status' => 'error', 'message' => 'Note not found or could not be deleted.']);
            }
        } else {
...
function getGradesInputUrl($class_id, $grading_system_type) {
    ...
    switch ($grading_system_type) {
        ...
        case 'a_na':
        case 'final_only':
        default: // Default to A/NA-based for any non-numerical system
...
$userNotes = [];
if (tableExists($conn, 'notes')) { // Assuming tableExists function is available and works as intended
    $sql_notes = "SELECT note_id, note_content FROM notes ORDER BY reg_date DESC"; // Fetch note_id as well
    $result_notes = $conn->query($sql_notes);
    ...
    if ($result_notes) {
        ...
        error_log("Error fetching notes: " . $conn->error);
    }
}
...
if (tableExists($conn, 'enrollments') && tableExists($conn, 'classes') && tableExists($conn, 'subjects') && tableExists($conn, 'sections')) {
    ...
    if ($stmt_students) {
        ...
        $result_students_per_class = $stmt_students->get_result();
        $studentsPerClass = $result_students_per_class->fetch_all(MYSQLI_ASSOC);
        $stmt_students->close();
    } else {
        error_log("Failed to prepare statement for getting student count per class: " . $conn->error);
...
<head>
    ...
    <title>Teacher Dashboard - Universidad De Manila</title>
...
<div class="main-wrapper">
    ...
    <main class="content-area">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <section class="mb-4">
                    <h4 style="color: #006400;">Your Notes</h4>
                </section>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form action="export_db.php" method="POST" class="d-inline">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // const saveDbButton = document.getElementById('saveDbButton'); // No longer needed for chatbot interaction
    ...
    // Event listener for the logout button to clear chat history
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <form action="export_db.php" method="POST" class="d-inline">
                    <button type="submit" class="btn btn-primary">Yes, Save Database</button>
                </form>
            </div>
        </div>
    </div>
